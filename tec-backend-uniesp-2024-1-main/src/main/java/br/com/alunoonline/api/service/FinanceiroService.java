...
public class FinanceiroService {
    ...
    private final FinanceiroAlunoRepository financeiroAlunoRepository;
    private final FaturaRepository faturaRepository;

    @Autowired
    public FinanceiroService(FinanceiroAlunoRepository financeiroAlunoRepository, FaturaRepository faturaRepository) {
        this.financeiroAlunoRepository = financeiroAlunoRepository;
    ...
    public void faturaGeneration() {
        ...
        for (FinanceiroAluno financeiroAluno : financeiroAlunos) {
            ...
            if (dueDay != null) {
                LocalDate dueDateCurrentMonth = getDueDateForCurrentOrNextMonth(dueDay, currentDate);

                if (shouldGenerateInvoice(dueDateCurrentMonth, generationDeadline)) {
                    generateInvoiceIfNotExists(financeiroAluno, dueDateCurrentMonth, currentDate);
                }
            }
        }
    ...
    private LocalDate getDueDateForCurrentOrNextMonth(Integer dueDay, LocalDateTime currentDate) {
        LocalDate dueDateCurrentMonth = calculateDueDate(dueDay, currentDate.getYear(), currentDate.getMonthValue());

        // Se a data de vencimento do mês atual já passou, calcular para o próximo mês
        ...
        return dueDateCurrentMonth;
    }

    private boolean shouldGenerateInvoice(LocalDate dueDateCurrentMonth, LocalDateTime generationDeadline) {
        return !dueDateCurrentMonth.isBefore(generationDeadline.toLocalDate()) && !dueDateCurrentMonth.isAfter(generationDeadline.toLocalDate());
    }

    private void generateInvoiceIfNotExists(FinanceiroAluno financeiroAluno, LocalDate dueDateCurrentMonth, LocalDateTime currentDate) {
